...
output_dir="./circpeptide4/AS_rMATS_jcast/netMHCpan/HLA_1/netMHCpan_1_result"
...
tmp_dir="./circpeptide4/AS_rMATS_jcast/netMHCpan/HLA_1/tmp"
...
run_netmhcpan() {
    ...
    IFS=$'\t' read -r task_index task_file_path task_file_name task_HLA_1 <<< "$1"
    ...
    if [[ -f "$output_file" ]]; then
        echo "[SKIP] ${task_index} | ${task_file_name} already completed"
        return 0
    fi

    # Prevent duplicate execution of the same task
    if ! (set -o noclobber; > "$running_flag") 2>/dev/null; then
        echo "[SKIP] ${task_index} | ${task_file_name} already running"
        return 0
    fi

    # Remove the running flag when the function exits
...
echo "============================================================"
echo "All netMHCpan tasks completed"
echo "============================================================"